...
const JournalIDE: React.FC<{ setView?: (view: ViewMode) => void }> = ({ setView }) => {
  ...
  // Fix: Explicitly type initialState to ensure the 'type' field is 'DEBIT' | 'CREDIT' and not widened to string
  ...
  const addLine = (type: 'DEBIT' | 'CREDIT') => {
    ...
    // Fix: cast the new object to JournalLine to satisfy type requirements
    setWorkingState({ ...workingState, lines: [...currentLines, { id: uuidv4(), type, accountId: '', accountName: '', code: '', amount: nextAmount } as JournalLine] });
  };
  ...
  const updateLine = (id: string, field: keyof JournalLine, value: any) => {
    // Fix: explicitly type the mapped result as JournalLine to prevent property inference errors
    const newLines = currentLines.map(line => line.id === id ? ({ ...line, [field]: value } as JournalLine) : line);
    setWorkingState({ ...workingState, lines: newLines });
  ...
  const handleSelectAccount = (lineId: string, acc: Account) => {
    // Fix: explicitly type the mapped result as JournalLine
    const newLines = currentLines.map(line => line.id === lineId ? ({ ...line, accountName: acc.name, accountId: acc.id, code: acc.code } as JournalLine) : line);
    setWorkingState({ ...workingState, lines: newLines });
  ...
  // Utility to prevent non-numeric input for financial fields
                    ...
                    <div className="flex items-center gap-3 md:gap-4 flex-1">
                      ...
                      <div className="flex-1 relative flex items-center gap-2 md:gap-3">
                        ...
                        <div className="flex items-center">
                          <button 
                            onClick={() => isNew && setRegistrationData({lineId: line.id, name: line.accountName})}
